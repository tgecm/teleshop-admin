...
export default function PublicEcommerce({ slug, viaDomain, mode }) {
  ...
  useEffect(() => {
    if (!shop?.id) return;
    fetch(API_BASE + '/public/quick-questions/' + shop.id)
      .then(res => res.json())
      .then(data => {
  ...
  const sendMessage = useCallback(async (msg) => {
    ...
    try {
      ...
      if (d.ai_disabled || d.ai_unavailable) {
        setChatMessages(prev => [...prev, { role: 'assistant', content: 'AI Agent is currently paused for this chat. Our shop staff will reply shortly!' }]);
      } else if (d.reply) {
  ...
  const handleQuickQuestionClick = useCallback((q) => {
    ...
    if (q.response_type === 'preset' && q.preset_answer) {
      setChatMessages(prev => [
        ...prev,
        { role: 'user', content: q.question, file_id: null, file_type: null },
