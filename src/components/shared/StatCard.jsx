...
export default function StatCard({ title, value, icon: Icon, trend, color = 'indigo' }) {
  ...
  return (
    ...
    >
      <div className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-br ${colors[color]} flex items-center justify-center text-white shadow-lg flex-shrink-0`}>
        <Icon className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-[9px] sm:text-xs md:text-sm font-bold text-gray-400 uppercase tracking-wider break-words">{title}</p>
        <h3 className="text-sm sm:text-base md:text-lg lg:text-xl font-bold text-gray-900 leading-tight mt-0.5 break-words">{value}</h3>
        {trend && (
