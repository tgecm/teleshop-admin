...
export default function TempAccountModal({ title, onSkip }) {
  ...
  return (
    <div className="py-2 sm:py-6 flex items-center justify-center px-2 sm:px-4 w-full my-auto">
      <motion.div
        initial={{ opacity: 0, scale: 0.96 }}
        animate={{ opacity: 1, scale: 1 }}
        className="max-w-md w-full bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-500/30 rounded-2xl sm:rounded-3xl p-4 sm:p-7 shadow-2xl space-y-3.5 sm:space-y-6 relative max-h-[80vh] sm:max-h-[85vh] overflow-y-auto scrollbar-none"
      >
        {/* Header */}
        <div className="text-center space-y-2 sm:space-y-3">
          <div className="w-12 h-12 sm:w-14 sm:h-14 mx-auto rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-500 shadow-inner">
          ...
          <p className="text-xs text-gray-500 dark:text-slate-400 break-all">
            Current login: <span className="text-amber-600 dark:text-amber-400 font-mono font-medium break-all">{user?.email}</span>
          </p>
        </div>

        {/* Form */}
        <form onSubmit={handleConfirmTempChange} className="space-y-3 sm:space-y-4">
          {/* Mail Input */}
          <div>
            <label className="block text-[11px] sm:text-xs font-bold text-gray-700 dark:text-slate-300 mb-1 uppercase tracking-wider">
              Enter your own mail
            </label>
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-4 py-2 sm:py-2.5 bg-gray-50 dark:bg-slate-800/80 border ${
                  tempEmailError
                    ? 'border-red-500 ring-1 ring-red-500'
                    : 'border-gray-200 dark:border-slate-700'
                } rounded-xl text-gray-900 dark:text-white text-xs sm:text-sm placeholder-gray-400 dark:placeholder-slate-500 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all`}
              />
            </div>
            {tempEmailError && (
              <p className="mt-1 text-xs font-medium text-red-500 dark:text-red-400 flex items-center gap-1">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-10 py-2 sm:py-2.5 bg-gray-50 dark:bg-slate-800/80 border ${
                  tempPwError
                    ? 'border-red-500 ring-1 ring-red-500'
                    : 'border-gray-200 dark:border-slate-700'
                } rounded-xl text-gray-900 dark:text-white text-xs sm:text-sm placeholder-gray-400 dark:placeholder-slate-500 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all`}
            ...
            {tempPwError ? (
              ...
              <p className="mt-1 text-[10px] sm:text-[11px] text-gray-500 dark:text-slate-400 flex items-center gap-1">
                <Info className="w-3 h-3 flex-shrink-0 text-amber-500" />
                Must have 1 Capital letter, 1 small letter, 1 number & min 8 chars.
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 sm:px-4 py-2 sm:py-2.5 bg-gray-50 dark:bg-slate-800/80 border border-gray-200 dark:border-slate-700 rounded-xl text-gray-900 dark:text-white text-xs sm:text-sm placeholder-gray-400 dark:placeholder-slate-500 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 tracking-widest text-center font-mono transition-all min-w-0"
              />
              <button
                type="button"
                onClick={handleGetTempCode}
                disabled={sendingTempCode || tempCodeCountdown > 0}
                className="px-3.5 py-2 sm:px-4 sm:py-2.5 bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 text-amber-600 dark:text-amber-300 text-xs font-bold rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all whitespace-nowrap flex items-center justify-center min-w-[85px] shrink-0"
              >
                {sendingTempCode ? (
          ...
          {/* Action Buttons */}
          <div className="flex items-center justify-between gap-2 pt-3 sm:pt-4 border-t border-gray-100 dark:border-slate-800">
            <button
              type="button"
              onClick={onSkip}
              className="px-3.5 py-2 sm:px-4 sm:py-2.5 text-xs font-bold text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-xl transition-all"
            >
              Not Now
            </button>
            <button
              ...
              className="px-5 py-2 sm:px-6 sm:py-2.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-xl shadow-lg shadow-amber-500/20 disabled:opacity-50 transition-all flex items-center gap-2"
            >
